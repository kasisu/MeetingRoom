...
<body>
	...
	<form action="<%=request.getContextPath()%>/ChangeDate" method="post">
		<label for="date"><input type="date" id="date" name="date"
			value="${meetingRoom.date}" required> </label><input type="hidden"
			name="page" value="reserveInput.jsp"> <input class="cursor"
			type="submit" value="日付変更" name="">
	</form>
	<h2>
		予約時間帯(<label for="name">${meetingRoom.user.name}</label>)
	</h2>
	<h2 class="error"><%=error%></h2>
	<div class="info">
		<input class='bcolor' type='submit' value='    ' disabled>予約可
		<input class='rcolor' type='submit' value='    ' disabled>予約済
		<input class='ncolor' type='submit' value='    ' disabled>予約不可
	</div>
	<table class="side">
		<%
		//リクエスト属性の取得
		...
		for (int j = 0; j < confrenceRoom.length; j++) {
			...
			out.println(confrenceRoom[j].getName());
			out.print("</td>");
			for (int i = 0; i < PERIOD.length; i++) {
				out.print("<td class='second'>");
				out.print("<form action='" + request.getContextPath() + "/ReserveCreate' method='post'>");
			...
			if (rbs[j][i] == null) {
				// 予約がない場合のタグ
				out.print("<input class='bcolor re' type='submit' value='    '>");
			} else {
				if (rbs[j][i].getUserId().equals(meetingRoom.getUser().getId())) {
					// ログインしているユーザーのタグ
					out.print("<input class='rcolor' type='submit' value='    ' disabled>");
				} else {
					out.print("<input class='ncolor' type='submit' value='    ' disabled>");
				}
			}
